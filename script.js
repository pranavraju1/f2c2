...
{"id":100,"first_name":"Zoe","last_name":"Moorhead","email":"zmoorhead2r@sfgate.com","gender":"Female","img_src":"https://robohash.org/fugiatcorporisdeleniti.png?size=50x50&set=set1","class":8,"marks":43,"passing":true,"city":"Makin Village"}]
      ...
      arr.forEach((item) => {
        ...
        row.insertCell(3).textContent = item.class;
        row.insertCell(4).textContent = item.marks;
        row.insertCell(5).textContent = item.passing ? "Passing" : "Failed";
        row.insertCell(6).textContent = item.email;
      });

      function populateTable(arr) {
        while (table.rows.length > 1 && flag) {
          table.deleteRow(1);
        }
        arr.forEach((item) => {
          let row = table.insertRow();
...
function pass(){
  let arr0 = arr.filter((a)=>{
    if(a.passing)return true;
  }); 
  return arr0;
}
